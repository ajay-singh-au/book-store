...
const BookDetails = (props) => {
  ...
  let exists = books.filter((book) => book.id === id)
  const showBook = books.find((book) => book.id === id)

  return exists.length ? (
    <div>
      <Link to="/">
        <button className="ml-2 text-left btn btn-warning">&lt; Back</button>
      </Link>
      <h1 className="text-center">Book Details with id - {props.match.params.book_id}</h1>
      <div className="text-white bg-dark d-flex justify-content-center align-items-center books-details-container mt-5">
        <ul>
          <li>
            <img src={showBook.thumbnailUrl} alt="" />
          </li>

          <li className="mt-4">
            <p>Book ID : {showBook.id}</p>
          </li>

          <li>
            <p>Book Title : {showBook.title}</p>
          </li>

          <li>
            <p>Author Name : {showBook.authors[0]}</p>
          </li>

          <li>
            <p>Categories : {showBook.categories}</p>
          </li>

          <li>
            <p>Status : {showBook.status}</p>
          </li>

          <li>
            <p>Total Pages: {showBook.pageCount}</p>
          </li>
          <li>
            <p>Description : {showBook.shortDescription}</p>
          </li>
        </ul>
      </div>
    </div>
    ...
    <div>
      ...
      <h1 className="mt-5 text-center text-danger">Invalid Book</h1>
    </div>
  )
}
